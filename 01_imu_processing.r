#Script for preparing the imu data for Safi et al 2025.
...
#input: IMU data is downloaded from Movebank (https://www.movebank.org")
#outputs: "your_path/your_processed_acc_data.rds", "your_path/your_processed_imu_data.rds"
...
#Data used in this study were downloaded on 15.04.2024. Use this timestamp as the cutoff point when downloading the data to reproduce the results of this study.
...
#first calculate burst duration
burst_size <- or %>% 
...
#IMU data is collected at a 20 Hz frequency and there are 1 or 1.2 seconds of data per row. In this step, I summarize these so that there is one second of data per row.
...
#for each second, calculate the mean, min, max, sd, cumsum for each axis. I end up only using a few of these metrics for the actual modeling 
